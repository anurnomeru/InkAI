...
- 能力：
  1) 生成前：根据图谱与章节号，汇总“必须延续/兑现”的约束，注入到 `build_chapter_prompt()`；
  2) 生成后：从本章文本抽取事件与关系，合并进图谱；
  3) 定稿前：运行一致性检查（未兑现伏笔、断因果链、时间顺序矛盾等），给出阻断/警告清单；
  4) 日志：把约束与检查要点输出到 UI 日志；
  5) 扩展：后续可基于该文件做可视化或高级查询（非本迭代范围）。
