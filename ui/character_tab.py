...
def build_character_tab(self):
    ...
    load_btn = ctk.CTkButton(self.character_tab, text=t("加载 character_state.txt"), command=self.load_character_state, font=("Microsoft YaHei", 12))
    load_btn.grid(row=0, column=0, padx=5, pady=5, sticky="w")

    self.character_wordcount_label = ctk.CTkLabel(self.character_tab, text=t("字数："), font=("Microsoft YaHei", 12))
    self.character_wordcount_label.grid(row=0, column=1, padx=5, pady=5, sticky="w")

    save_btn = ctk.CTkButton(self.character_tab, text=t("保存修改"), command=self.save_character_state, font=("Microsoft YaHei", 12))
    ...
    install_text_shortcuts(self.character_text)
    
    def update_word_count(event=None):
        text = self.character_text.get("0.0", "end-1c")
        text_length = len(text)
        self.character_wordcount_label.configure(text=t("字数：{n}").format(n=text_length))
...
def load_character_state(self):
    ...
    self.log("已加载 character_state.txt 鍒扮紪杈戝尯")
...
def save_character_state(self):
    ...
    self.log("已保存樺 character_state.txt 鐨勪慨鏀广€")
