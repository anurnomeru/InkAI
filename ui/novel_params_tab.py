...
def build_novel_params_area(self, start_row=1):
    ...
    genre_entry = ctk.CTkEntry(
        self.params_frame, textvariable=self.genre_var, font=("Microsoft YaHei", 12)
    )
    ...
    chapter_word_frame.grid(
        row=row_for_chapter_and_word, column=1, padx=5, pady=5, sticky="ew"
    )

    chapter_word_frame.columnconfigure((0, 1, 2, 3), weight=0)

    num_chapters_label = ctk.CTkLabel(
        chapter_word_frame, text=t("章节数:"), font=("Microsoft YaHei", 12)
    )
    ...
    word_number_label = ctk.CTkLabel(
        chapter_word_frame, text=t("每章字数:"), font=("Microsoft YaHei", 12)
    )
    ...
    label_draft_variants = ctk.CTkLabel(
        self.params_frame, text=t("草稿变体个数"), font=("Microsoft YaHei", 12)
    )
    label_draft_variants.grid(
        row=row_draft_variants, column=0, padx=5, pady=5, sticky="e"
    )
    ...
    draft_variants_entry.grid(
        row=row_draft_variants, column=1, padx=5, pady=5, sticky="w"
    )
    ...
    self.user_guide_text.grid(
        row=row_user_guide, column=1, padx=5, pady=5, sticky="nsew"
    )
    ...
    self.char_inv_text = ctk.CTkTextbox(
        char_inv_frame, height=60, wrap="word", font=("Microsoft YaHei", 12)
    )
...
def build_optional_buttons_area(self, start_row=2):
    ...
    self.btn_open_embed_dashboard = ctk.CTkButton(
        self.optional_btn_frame,
        text=t("打开向量工作台"),
        command=self.open_embed_dashboard_ui,
        font=("Microsoft YaHei", 12),
        width=120,
    )
...
def create_label_with_help_for_novel_params(
    ...
    btn = ctk.CTkButton(
        ...
        command=lambda: messagebox.showinfo(
            "鍙傛暟璇存槑", tooltips.get(tooltip_key, "鏆傛棤璇存槑")
        ),
    )
