...
from novel_generator.character_store import ensure_structure, load_index, build_effective
...
def open_character_select_dialog(self):
    ...
    ctk.CTkLabel(bar, text='搜索', font=DEFAULT_FONT).pack(side='left')
    ...
    def refresh_list():
        ...
        # 排序：最近更新在前
        items.sort(key=lambda it: (it.get('updated_at',''), it.get('name','')), reverse=True)
        for it in items:
            ...
            ctk.CTkCheckBox(row, text=name, variable=var, font=DEFAULT_FONT).pack(side='left', padx=4)
            # 预览按钮
            def _mk_preview(nm=name, cid=it.get('id') or name):
                def _show():
                    try:
                        ...
                        txt.append(f"姓名: {eff.get('name')}")
                        if eff.get('summary'):
                            txt.append('摘要: ' + str(eff.get('summary')))
                        if eff.get('attributes'):
                            txt.append('属性: ' + ', '.join(eff.get('attributes').keys()))
                        messagebox.showinfo('预览', '\n'.join(txt), parent=top)
                    except Exception:
                        messagebox.showinfo('预览', f'{nm}', parent=top)
                return _show
            ctk.CTkButton(row, text='预览', width=60, command=_mk_preview()).pack(side='right', padx=4)
    ...
    def do_confirm():
        ...
        try:
            if not sel:
                messagebox.showinfo('提示','未选择任何角色', parent=top)
                return
            # 回填至“参与角色”文本/变量
            joined = ','.join(sel)
            if hasattr(self, 'characters_involved_var') and self.characters_involved_var:
                try:
    ...
    ctk.CTkButton(btns, text='刷新', width=80, command=refresh_list).pack(side='left', padx=4)
    ctk.CTkButton(btns, text='确定', width=80, command=do_confirm).pack(side='right', padx=4)
    ctk.CTkButton(btns, text='取消', width=80, command=top.destroy).pack(side='right', padx=4)
