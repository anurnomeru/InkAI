...
def build_directory_tab(self):
    ...
    load_btn = ctk.CTkButton(self.directory_tab, text=t("加载 Novel_directory.txt"), command=self.load_chapter_blueprint, font=("Microsoft YaHei", 12))
    load_btn.grid(row=0, column=0, padx=5, pady=5, sticky="w")

    self.directory_word_count_label = ctk.CTkLabel(self.directory_tab, text=t("字数："), font=("Microsoft YaHei", 12))
    self.directory_word_count_label.grid(row=0, column=1, padx=5, pady=5, sticky="w")

    save_btn = ctk.CTkButton(self.directory_tab, text=t("保存修改"), command=self.save_chapter_blueprint, font=("Microsoft YaHei", 12))
    ...
    install_text_shortcuts(self.directory_text)
    
    def update_word_count(event=None):
        text = self.directory_text.get("0.0", "end")
        count = len(text) - 1
        self.directory_word_count_label.configure(text=t("字数：{n}").format(n=count))
...
def load_chapter_blueprint(self):
    ...
    self.log("已加载 Novel_directory.txt 鍐呭鍒扮紪杈戝尯")
...
def save_chapter_blueprint(self):
    ...
    self.log("已保存樺 Novel_directory.txt 鐨勪慨鏀广€")
