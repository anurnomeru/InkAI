...
FIELDS = ['name','aliases','tags','summary','attributes','relationships','timeline']
...
def open_character_review_dialog(self):
    ...
    ctk.CTkLabel(bar, text='选择章节', font=DEFAULT_FONT).pack(side='left')
    ...
    chap_menu = ctk.CTkOptionMenu(bar, values=chap_values or [''], variable=chap_var, width=120)
    ...
    def render():
        ...
        if not sel or not sel.isdigit():
            ctk.CTkLabel(list_frame, text='无可审阅条目', font=DEFAULT_FONT).pack(pady=12)
            return
        chap = int(sel)
        ids = chapters_map.get(chap, [])
        if not ids:
            ctk.CTkLabel(list_frame, text=f'第{chap}章：无自动抽取条目', font=DEFAULT_FONT).pack(pady=12)
            return
        for cid in ids:
            card = ctk.CTkFrame(list_frame)
            card.pack(fill='x', padx=4, pady=4)
            title = ctk.CTkLabel(card, text=f'角色：{cid}', font=("Microsoft YaHei", 13))
            ...
            per = {}
            for f in FIELDS:
                var = ctk.BooleanVar(value=(f in ('relationships','timeline')))  # 默认推荐采纳关系/时间线
                ctk.CTkCheckBox(row, text=f, variable=var).pack(side='left', padx=6)
                per[f] = var
            entries_vars[cid] = per
            # 说明
            tip = ctk.CTkLabel(card, text=f"手动已有摘要：{'是' if manual.get('summary') else '否'}；手动属性数：{len((manual.get('attributes') or {}).keys())}", font=("Microsoft YaHei", 11))
    ...
    def do_adopt():
        sel = chap_var.get().strip()
        if not sel or not sel.isdigit():
            messagebox.showwarning('提示','请选择章节', parent=top)
            return
        chap = int(sel)
        adopted = 0
        ...
        messagebox.showinfo('完成', f'已采纳 {adopted} 个角色的所选字段', parent=top)

    # 底部按钮
    btns = ctk.CTkFrame(top)
    btns.pack(fill='x', padx=8, pady=(0,8))
    ctk.CTkButton(btns, text='刷新', width=80, command=do_reload).pack(side='left', padx=4)
    ctk.CTkButton(btns, text='采纳所选', width=100, command=do_adopt).pack(side='right', padx=4)
    ctk.CTkButton(btns, text='关闭', width=80, command=top.destroy).pack(side='right', padx=4)
