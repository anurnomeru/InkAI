...
def build_ai_config_tab(self):
    ...
    def on_config_selected(new_value):
        ...
        if new_value in self.loaded_config.get("llm_configs", {}):
            ...
            # 更新显示标签
            self.temp_value_label.configure(
                text=f"{float(config.get('temperature', 0.7)):.2f}"
            )
            self.max_tokens_value_label.configure(
                text=str(int(config.get("max_tokens", 8192)))
            )
            self.timeout_value_label.configure(
                text=str(int(config.get("timeout", 600)))
            )
            # 同步模型控件显隐（Entry/OptionMenu）
            sync_model_control()
            # 同步 OpenCode Agent（如配置中保存了 agent）
            try:
                self.opencode_agent_var.set(config.get("agent", ""))
            except Exception:
                pass

    def add_new_config():
        """添加新配置 - 弹出对话框让用户输入名称"""
        dialog = ctk.CTkInputDialog(text="请输入新配置名称:", title="新增配置")
    ...
    def delete_current_config():
        ...
        if selected_config in self.loaded_config.get("llm_configs", {}):
            ...
            confirm = messagebox.askyesno(
                "确认删除", f"确定要删除配置 '{selected_config}' 吗？\n此操作不可撤销!"
            )
        if not confirm:
            return
        ...
        try:
            ...
            messagebox.showinfo(
                "提示", f"已删除配置: {selected_config}，并已更新配置文件"
            )
    ...
    def rename_current_config():
        ...
        dialog = ctk.CTkInputDialog(
            text=f"请输入新的配置名称 (原名称: {old_name}):", title="重命名配置"
        )
        new_name = dialog.get_input()
    ...
    self.ai_config_tab.grid_columnconfigure(3, weight=0)

    # 配置选择控件
    create_label_with_help(
        self, self.ai_config_tab, "当前配置", "interface_config", 0, 0
    )
    config_names = list(self.loaded_config.get("llm_configs", {}).keys())
    if not config_names:
        self.loaded_config["llm_configs"] = {
    ...
    interface_config_dropdown.grid(
        row=0, column=1, columnspan=2, padx=5, pady=5, sticky="nsew"
    )

    # 配置管理按钮组
    btn_frame = ctk.CTkFrame(self.ai_config_tab)
    btn_frame.grid(row=1, column=0, columnspan=4, padx=5, pady=5, sticky="ew")
    ...
    # 1) API Key
    create_label_with_help(
        self, self.ai_config_tab, "API Key:", "api_key", row_start, 0
    )
    ...
    api_key_entry.grid(
        row=row_start, column=1, columnspan=3, padx=5, pady=5, sticky="nsew"
    )

    # 2) Base URL
    create_label_with_help(
        self, self.ai_config_tab, "Base URL:", "base_url", row_start + 1, 0
    )
    self.base_url_var = ctk.StringVar(value="")
    base_url_entry = ctk.CTkEntry(
        self.ai_config_tab, textvariable=self.base_url_var, font=("Microsoft YaHei", 12)
    )
    base_url_entry.grid(
        row=row_start + 1, column=1, columnspan=3, padx=5, pady=5, sticky="nsew"
    )

    # 3) 接口格式
    create_label_with_help(
        self, self.ai_config_tab, "接口格式:", "interface_format", row_start + 2, 0
    )
    ...
    # 缓存（仅文件 -> 内存）
    if not hasattr(self, "_opencode_models"):
        self._opencode_models = []
    if not hasattr(self, "_opencode_agents"):
        self._opencode_agents = []
    # 启动时尝试从文件缓存加载（不触网）
    try:
        from llm_adapters import get_cached_opencode_models, get_cached_opencode_agents

        cached_models = get_cached_opencode_models() or []
        cached_agents = get_cached_opencode_agents() or []
        if cached_models:
            self._opencode_models = cached_models
        if cached_agents:
            self._opencode_agents = cached_agents
    except Exception:
        pass
    ...
    def on_interface_format_changed(new_value):
        try:
            if new_value.strip().lower() == "opencode":
                # 启用刷新按钮，但不主动刷新；优先用缓存
                for btn in [
                    getattr(self, "opencode_refresh_btn", None),
                    getattr(self, "opencode_agent_refresh_btn", None),
                ]:
                    try:
                        if btn is not None:
                ...
                for btn in [
                    ...
                    try:
                        ...
                        pass
                sync_model_control()
                # 非 OpenCode 隐藏 Agent 控件
                try:
                    self.opencode_agent_menu.grid_remove()
                    self.opencode_agent_entry.grid_remove()
                    self.opencode_agent_refresh_btn.configure(state="disabled")
                except Exception:
                    pass
        except Exception:
            pass
    ...
    interface_dropdown.grid(
        row=row_start + 2, column=1, columnspan=2, padx=5, pady=5, sticky="nsew"
    )

    self.opencode_refresh_btn = ctk.CTkButton(
        self.ai_config_tab,
        text="刷新 OpenCode 模型",
        command=_refresh_opencode_models,
        font=("Microsoft YaHei", 12),
    )
    self.opencode_refresh_btn.grid(
        row=row_start + 2, column=3, padx=5, pady=5, sticky="ew"
    )

    # 4) Model Name
    create_label_with_help(
        self, self.ai_config_tab, "模型名称:", "model_name", row_start + 3, 0
    )
    ...
    self.model_name_menu = ctk.CTkOptionMenu(
        self.ai_config_tab,
        values=[],
    ...
    self.model_name_entry.grid(
        row=row_start + 3, column=1, columnspan=3, padx=5, pady=5, sticky="nsew"
    )

    def sync_model_control():
        try:
            fmt = (self.interface_format_var.get() or "").strip().lower()
            if fmt == "opencode":
                try:
                    self.model_name_entry.grid_forget()
                except Exception:
                    pass
                # 若缓存有值，配置下拉
                if self._opencode_models:
                    try:
                        self.model_name_menu.configure(values=self._opencode_models)
                        if (
                            self.model_name_var.get() or ""
                        ).strip() not in self._opencode_models:
                            self.model_name_var.set(self._opencode_models[0])
                    except Exception:
                        pass
                self.model_name_menu.grid(
                    row=row_start + 3,
                    column=1,
                    columnspan=3,
                    padx=5,
                ...
                try:
                    self.opencode_refresh_btn.configure(state="normal")
                except Exception:
                    pass
            else:
                try:
                    self.model_name_menu.grid_forget()
                except Exception:
                    pass
                self.model_name_entry.grid(
                    row=row_start + 3,
                ...
                try:
                    self.opencode_refresh_btn.configure(state="disabled")
                except Exception:
                    pass
        except Exception:
            pass
    ...
    # Agent 选择（OpenCode 专用）
    create_label_with_help(
        self, self.ai_config_tab, "Agent:", "interface_config", row_start + 4, 0
    )
    self.opencode_agent_var = ctk.StringVar(value=os.environ.get("OPENCODE_AGENT", ""))

    def _on_agent_changed(v: str):
        try:
            v = (v or "").strip()
            if v:
                os.environ["OPENCODE_AGENT"] = v
            else:
    ...
    # 下拉：用于有列表时
    self.opencode_agent_menu = ctk.CTkOptionMenu(
        self.ai_config_tab,
        values=[],
        variable=self.opencode_agent_var,
        command=_on_agent_changed,
        font=("Microsoft YaHei", 12),
    )
    # 输入：用于无法获取列表时
    self.opencode_agent_entry = ctk.CTkEntry(
        self.ai_config_tab,
        textvariable=self.opencode_agent_var,
        font=("Microsoft YaHei", 12),
    )

    def _on_manual_agent_change(*_):
        _on_agent_changed(self.opencode_agent_var.get())

    try:
        self.opencode_agent_var.trace_add("write", _on_manual_agent_change)
    except Exception:
        pass

    # 刷新按钮
    self.opencode_agent_refresh_btn = ctk.CTkButton(
        self.ai_config_tab,
        text="刷新 OpenCode Agent",
        command=_refresh_opencode_agents,
        font=("Microsoft YaHei", 12),
    )

    def sync_agent_control():
        try:
            fmt = (self.interface_format_var.get() or "").strip().lower()
            if fmt != "opencode":
                try:
                ...
                try:
                    ...
                    pass
                try:
                    self.opencode_agent_refresh_btn.grid_remove()
                except Exception:
                    pass
                return
            # 确保刷新按钮在位（不主动刷新，除非用户点击）
            self.opencode_agent_refresh_btn.grid(
                row=row_start + 4, column=3, padx=5, pady=5, sticky="ew"
            )
            # 有缓存 → 下拉；否则 → 输入框
            if self._opencode_agents:
                try:
                ...
                try:
                    ...
                    pass
                if (
                    self.opencode_agent_var.get() or ""
                ).strip() not in self._opencode_agents:
                    self.opencode_agent_var.set(self._opencode_agents[0])
                self.opencode_agent_menu.grid(
                    row=row_start + 4,
                    column=1,
                    columnspan=2,
                    padx=5,
                ...
                try:
                    ...
                    pass
                self.opencode_agent_entry.grid(
                    row=row_start + 4,
                    column=1,
                    columnspan=2,
                    padx=5,
    ...
    # 5) Temperature
    create_label_with_help(
        self, self.ai_config_tab, "Temperature:", "temperature", row_start + 5, 0
    )
    ...
    temp_scale.grid(row=row_start + 5, column=1, padx=5, pady=5, sticky="we")
    self.temp_value_label = ctk.CTkLabel(
        self.ai_config_tab,
        text=f"{self.temperature_var.get():.2f}",
        font=("Microsoft YaHei", 12),
    )
    self.temp_value_label.grid(row=row_start + 5, column=2, padx=5, pady=5, sticky="w")

    # 6) Max Tokens
    create_label_with_help(
        self, self.ai_config_tab, "Max Tokens:", "max_tokens", row_start + 6, 0
    )
    ...
    max_tokens_slider.grid(row=row_start + 6, column=1, padx=5, pady=5, sticky="we")
    self.max_tokens_value_label = ctk.CTkLabel(
        self.ai_config_tab,
        text=str(self.max_tokens_var.get()),
        font=("Microsoft YaHei", 12),
    )
    self.max_tokens_value_label.grid(
        row=row_start + 6, column=2, padx=5, pady=5, sticky="w"
    )

    # 7) Timeout
    create_label_with_help(
        self, self.ai_config_tab, "Timeout (sec):", "timeout", row_start + 7, 0
    )
    ...
    timeout_slider.grid(row=row_start + 7, column=1, padx=5, pady=5, sticky="we")
    self.timeout_value_label = ctk.CTkLabel(
        self.ai_config_tab,
        text=str(self.timeout_var.get()),
        font=("Microsoft YaHei", 12),
    )
    self.timeout_value_label.grid(
        row=row_start + 7, column=2, padx=5, pady=5, sticky="w"
    )
    ...
    test_btn.grid(
        row=row_start + 8, column=0, columnspan=4, padx=5, pady=5, sticky="ew"
    )
...
def build_embeddings_config_tab(self):
    def on_embedding_interface_changed(new_value):
        ...
        if (
            self.loaded_config
            and "embedding_configs" in self.loaded_config
            and new_value in self.loaded_config["embedding_configs"]
        ):
            ...
            self.embedding_url_var.set(
                emb_conf.get("base_url", self.embedding_url_var.get())
            )
            ...
            if new_value == "Ollama":
                ...
                self.embedding_url_var.set(
                    "https://[az].openai.azure.com/openai/deployments/[model]/embeddings?api-version=2023-05-15"
                )
    ...
    create_label_with_help(
        ...
        tooltip_key="embedding_intexrface_format",
        row=1,
        column=0,
        font=("Microsoft YaHei", 12),
    )

    emb_interface_options = [
        "DeepSeek",
        "OpenAI",
        "Azure OpenAI",
        "Gemini",
...
def build_config_choose_tab(self):
    ...
    create_label_with_help(
        self,
        parent=self.config_choose,
        label_text="生成架构所用大模型",
        tooltip_key="architecture_llm_config",
        row=0,
        column=0,
        font=("Microsoft YaHei", 12),
    )
    architecture_dropdown = ctk.CTkOptionMenu(
        self.config_choose,
        values=config_choose_options,
        variable=self.architecture_llm_var,
        font=("Microsoft YaHei", 12),
    )
    ...
    create_label_with_help(
        self,
        parent=self.config_choose,
        label_text="生成大目录所用大模型",
        tooltip_key="chapter_outline_llm_config",
        row=1,
        column=0,
        font=("Microsoft YaHei", 12),
    )
    chapter_outline_dropdown = ctk.CTkOptionMenu(
        self.config_choose,
        values=config_choose_options,
        variable=self.chapter_outline_llm_var,
        font=("Microsoft YaHei", 12),
    )
    ...
    create_label_with_help(
        self,
        parent=self.config_choose,
        label_text="生成草稿所用大模型",
        tooltip_key="prompt_draft_llm_config",
        row=2,
        column=0,
        font=("Microsoft YaHei", 12),
    )
    prompt_draft_dropdown = ctk.CTkOptionMenu(
        self.config_choose,
        values=config_choose_options,
        variable=self.prompt_draft_llm_var,
        font=("Microsoft YaHei", 12),
    )
    ...
    create_label_with_help(
        self,
        parent=self.config_choose,
        label_text="定稿章节所用大模型",
        tooltip_key="final_chapter_llm_config",
        row=3,
        column=0,
        font=("Microsoft YaHei", 12),
    )
    final_chapter_dropdown = ctk.CTkOptionMenu(
        self.config_choose,
        values=config_choose_options,
        variable=self.final_chapter_llm_var,
        font=("Microsoft YaHei", 12),
    )
    ...
    create_label_with_help(
        self,
        parent=self.config_choose,
        label_text="一致性审校所用大模型",
        tooltip_key="consistency_review_llm_config",
        row=4,
        column=0,
        font=("Microsoft YaHei", 12),
    )
    consistency_review_dropdown = ctk.CTkOptionMenu(
        self.config_choose,
        values=config_choose_options,
        variable=self.consistency_review_llm_var,
        font=("Microsoft YaHei", 12),
    )
    ...
    create_label_with_help(
        self,
        parent=self.config_choose,
        label_text="使用的 Embedding 配置",
        tooltip_key="embedding_choice",
        row=5,
        column=0,
        font=("Microsoft YaHei", 12),
    )
    if not hasattr(self, "embedding_choice_var"):
        ...
        last_embedding = self.loaded_config.get(
            "last_embedding_interface_format", "OpenAI"
        )
        self.embedding_choice_var = ctk.StringVar(value=last_embedding)
    embedding_choice_dropdown = ctk.CTkOptionMenu(
        self.config_choose,
        values=embed_names,
        variable=self.embedding_choice_var,
        font=("Microsoft YaHei", 12),
    )
    ...
    def refresh_config_dropdowns():
        ...
        for dropdown in [
            ...
            chapter_outline_dropdown,
            prompt_draft_dropdown,
            final_chapter_dropdown,
            consistency_review_dropdown,
        ]:
...
def build_proxy_setting_tab(self):
    ...
    # 代理启用开关
    create_label_with_help(
        self, self.proxy_setting_tab, "启用代理:", "proxy_enabled", 0, 0
    )
    ...
    self.proxy_address_var = ctk.StringVar(
        value=proxy_setting.get("proxy_url", "127.0.0.1")
    )
...
def load_config_btn(self):
    ...
    if cfg:
        ...
        if last_embedding in embedding_configs:
            ...
            self.embedding_api_key_var.set(emb_conf.get("api_key", ""))
            self.embedding_url_var.set(
                emb_conf.get("base_url", "https://api.openai.com/v1")
            )
            self.embedding_model_name_var.set(
                emb_conf.get("model_name", "text-embedding-ada-002")
            )
            self.embedding_retrieval_k_var.set(str(emb_conf.get("retrieval_k", 4)))
