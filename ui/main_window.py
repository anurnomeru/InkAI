...
from ui.chapters_tab import (
    build_chapters_tab,
    refresh_chapters_list,
    on_chapter_selected,
...
class NovelGeneratorGUI:
    ...
    def __init__(self, master):
        ...
        if self.loaded_config:
            last_llm = next(iter(self.loaded_config["llm_configs"].values())).get(
        ...
        if (
            ...
            and last_embedding in self.loaded_config["embedding_configs"]
        ):
        ...
        self.base_url_var = ctk.StringVar(
            value=llm_conf.get("base_url", "https://api.openai.com/v1")
        )

        self.interface_format_var = ctk.StringVar(
            value=llm_conf.get("interface_format", "OpenAI")
        )

        self.model_name_var = ctk.StringVar(
            value=llm_conf.get("model_name", "gpt-4o-mini")
        )
        ...
        self.interface_config_var = ctk.StringVar(
            value=next(iter(self.loaded_config["llm_configs"]))
        )
        ...
        self.embedding_url_var = ctk.StringVar(
            value=emb_conf.get("base_url", "https://api.openai.com/v1")
        )

        self.embedding_model_name_var = ctk.StringVar(
            value=emb_conf.get("model_name", "text-embedding-ada-002")
        )

        self.embedding_retrieval_k_var = ctk.StringVar(
            value=str(emb_conf.get("retrieval_k", 4))
        )

        # -- 生成配置鐩稿叧 --

        self.architecture_llm_var = ctk.StringVar(
            value=choose_configs.get("architecture_llm", "DeepSeek")
        )

        self.chapter_outline_llm_var = ctk.StringVar(
            value=choose_configs.get("chapter_outline_llm", "DeepSeek")
        )

        self.final_chapter_llm_var = ctk.StringVar(
            value=choose_configs.get("final_chapter_llm", "DeepSeek")
        )

        self.consistency_review_llm_var = ctk.StringVar(
            value=choose_configs.get("consistency_review_llm", "DeepSeek")
        )

        self.prompt_draft_llm_var = ctk.StringVar(
            value=choose_configs.get("prompt_draft_llm", "DeepSeek")
        )
        ...
        if self.loaded_config and "other_params" in self.loaded_config:
            ...
            self.draft_variants_var = ctk.StringVar(
                value=str(op.get("draft_variants", 3))
            )

            self.characters_involved_var = ctk.StringVar(
                value=op.get("characters_involved", "")
            )
            ...
            self.webdav_username_var = ctk.StringVar(
                value=op.get("webdav_username", "")
            )

            self.webdav_password_var = ctk.StringVar(
                value=op.get("webdav_password", "")
            )
        ...
        def _run_build_steps():
            ...
            steps.append(lambda: build_other_settings_tab(self))
            ...
            def _next():
                ...
                try:
                    ...
                    try:
                        if hasattr(self, "handle_exception"):
                            self.handle_exception("Build UI step failed")
                    except Exception:
                        pass
                # 让出事件循环
                self.master.after(0, _next)

            _next()

        # 提供一个安全封装，避免 early init 时 AttributeError
        def _safe_update_vector_btn():
            try:
        ...
        # 绑定到实例上供 steps 调用
        self._safe_update_vector_btn = _safe_update_vector_btn
        # 启动分步构建
        self.master.after(0, _run_build_steps)

        try:
            # 章节号变更 -> 刷新变体并加载对应内容
            self.chapter_num_var.trace_add(
                "write", lambda *a: self._on_chapter_num_changed()
            )
        except Exception:
            pass
        try:
            self.chapter_num_var.trace_add(
                "write", lambda *a: self.update_finalized_status_label()
            )
        except Exception:
            pass
        try:
    ...
    def update_finalized_status_label(self):
        try:
            fp = (
                (self.filepath_var.get() or "").strip()
            ...
            if not (
                hasattr(self, "finalized_hint_label")
                or hasattr(self, "finalized_hint_label_params")
            ):
                return
            if not (fp and chap):
            ...
            for b in btns:
                try:
                    if empty:
                        b.configure(
                            text=t("重建向量库"),
                            command=self.rebuild_full_vectorstore_ui,
                        )
                    else:
                        b.configure(
                            text=t("清空向量库"),
    ...
    def show_character_import_window(self):
        ...
        if os.path.exists(role_lib_path):
            ...
            for category in os.listdir(role_lib_path):
                ...
                if os.path.isdir(category_path):
                    ...
                    category_frame.grid(
                        row=current_row, column=0, sticky="w", pady=(10, 5), padx=5
                    )
                    ...
                    for role_file in os.listdir(category_path):
                        if role_file.endswith(".txt"):
                            ...
                            if not any(
                                name == role_name for _, name in self.selected_roles
                            ):
                    ...
                    if role_count == 0:
                        category_label.grid(
                            columnspan=max_roles_per_row + 1, sticky="w"
                        )
        ...
        btn_cancel = ctk.CTkButton(
            btn_frame, text=t("取消"), command=import_window.destroy
        )
        ...
        if RoleLibrary is None:
            try:
                ...
                messagebox.showerror(
                    t("错误"), t("角色库模块加载失败：{err}").format(err=str(e))
                )
                return
    ...
    def save_all_config(self):
        try:
            ...
            llm_item = {
                "api_key": (
                    self.api_key_var.get() if hasattr(self, "api_key_var") else ""
                ).strip(),
                "base_url": (
                    self.base_url_var.get() if hasattr(self, "base_url_var") else ""
                ).strip(),
                "model_name": (
                    self.model_name_var.get() if hasattr(self, "model_name_var") else ""
                ).strip(),
                "temperature": float(self.temperature_var.get())
                if hasattr(self, "temperature_var")
                else 0.7,
                "max_tokens": _safe_int(self.max_tokens_var, 8192)
                if hasattr(self, "max_tokens_var")
                else 8192,
                "timeout": _safe_int(self.timeout_var, 600)
            ...
            emb_if = (
                ...
                if hasattr(self, "embedding_interface_format_var")
                else "OpenAI"
            )
            emb_item = {
                "api_key": (
                    self.embedding_api_key_var.get()
                    if hasattr(self, "embedding_api_key_var")
                    else ""
                ).strip(),
                "base_url": (
                    self.embedding_url_var.get()
                    if hasattr(self, "embedding_url_var")
                    else ""
                ).strip(),
                "model_name": (
                    self.embedding_model_name_var.get()
                    if hasattr(self, "embedding_model_name_var")
                    else ""
                ).strip(),
                "retrieval_k": _safe_int(self.embedding_retrieval_k_var, 4)
                if hasattr(self, "embedding_retrieval_k_var")
                else 4,
            ...
            other = {
                ...
                "num_chapters": _safe_int(self.num_chapters_var, 1)
                if hasattr(self, "num_chapters_var")
                else 1,
                "word_number": _safe_int(self.word_number_var, 10000)
                if hasattr(self, "word_number_var")
                else 10000,
                "filepath": self.filepath_var.get()
                if hasattr(self, "filepath_var")
                else "",
                "chapter_num": str(self.chapter_num_var.get())
                if hasattr(self, "chapter_num_var")
                else "1",
                ...
                "draft_variants": _safe_int(self.draft_variants_var, 3)
